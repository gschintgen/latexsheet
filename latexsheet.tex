...
\settowidth{\MyLen}{\texttt{mathtools} }
\begin{tabular}{@{}p{\the\MyLen}%
                @{}p{\linewidth-\the\MyLen}@{}}
\texttt{multicol}  &  Use $n$ columns: 
                        \verb!\begin{multicols}{!$n$\verb!}!.   \\
\texttt{mathtools}  &  Advanced math typesetting. Includes \texttt{amsmath}.\\
\texttt{geometry}  &  Page layout (margins, etc.)\\
\texttt{fancyhdr}  &  Customize header and footer\\
\texttt{graphicx}  &  Show image:
                        \verb!\includegraphics[width=!%
                        \textit{x}\verb!]{!%
                        \textit{file}\verb!}!. \\
\texttt{hyperref}       & Insert URL: \verb!\url{!%
                        \textit{https://\ldots}%
                        \verb!}!\newline or hyperlink: \verb!\href{URL}{text}!.\\
\texttt{enumitem}         & Customize \verb!enumerate!.\\
\texttt{tabularx}         & Stretchable \texttt{X} columns in tables.\\
\texttt{tabulary}         & Automatically balanced column widths.\\
\texttt{tabto}          & Tab stops (compatible with enumerations).\\
\texttt{beamer}         & For creating slides.
\end{tabular}
...
\url{https://github.com/gschintgen/latexsheet}
